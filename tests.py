...
class TestPythonDisksorted(unittest.TestCase):
    ...
    def test_medium(self):
        ...
        self.assertEqual(list(disksorted(initial, chunksize=1000)), range(10000))

    def test_objects(self):
        initial = [TestClass(i) for i in range(1000)]
        random.shuffle(initial)
        result = [i.value for i in disksorted(initial, chunksize=500, key=lambda x: x.value)]
        self.assertEqual(result, range(1000))

    def test_namedtuple(self):
        initial = [TestNamedTuple(i) for i in range(1000)]
        random.shuffle(initial)
        result = [i.value for i in disksorted(initial, chunksize=500, key=lambda x: x.value)]
        self.assertEqual(result, range(1000))
    
    def test_serialize_json(self):
        self.assertEqual(list(disksorted(range(10), chunksize=4, serializer=SERIALIZER_JSON)), range(10))
    ...
    def test_serialize_marshal(self):
        self.assertEqual(list(disksorted(range(10), chunksize=4, serializer=SERIALIZER_MARSHAL)), range(10))
